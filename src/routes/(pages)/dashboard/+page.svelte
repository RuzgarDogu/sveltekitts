...
<p class="mb-5 text-lg font-medium text-gray-900 dark:text-white">Choose technology:
	{#if selected_markets.length > 0}
	<span class="mb-5 text-lg font-medium text-gray-900 dark:text-white">{selected_markets.length}</span>
	{/if}
</p>
	...
	<Checkbox bind:group={selected_markets} value={market.id} custom>
		<div class="font-normal p-5 w-full text-gray-500 bg-white rounded-lg border-2 border-gray-200 cursor-pointer dark:hover:text-gray-300 dark:border-gray-700 peer-checked:border-blue-600 hover:text-gray-600 dark:peer-checked:text-gray-300 peer-checked:text-gray-600 hover:bg-gray-50 dark:text-gray-400 dark:bg-gray-800 dark:hover:bg-gray-700">
			...
			<div class="flex justify-between">
				...
				<a href={market.url} class="bg-slate-700 hover:bg-slate-900 focus:outline-none focus:ring-2 focus:ring-slate-400 focus:ring-offset-2 focus:ring-offset-slate-50 text-white font-semibold h-8 px-6 rounded-lg w-full flex items-center justify-center sm:w-auto dark:bg-sky-500 dark:highlight-white/20 dark:hover:bg-sky-400">Ziyaret Et</a>
			</div>
		</div>
	</Checkbox>
	{/each}
